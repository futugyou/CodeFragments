...
namespace KaleidoCode.KernelService.Skills;
...
public class WebSearch(IOptionsMonitor<SemanticKernelOptions> optionsMonitor)
{
    ...
    public async Task<List<string>> SimpleWebSearchAsync(string engine = "google", string query = "", int count = 1)
    {
        ...
        // System.NotSupportedException: The type 'KaleidoCode.KernelService.Skills.WebSearch+<CommonWebSearchAsync>d__2' can only be serialized using async serialization methods. Path: $.
