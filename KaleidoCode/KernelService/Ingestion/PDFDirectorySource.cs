...
namespace KaleidoCode.KernelService.Ingestion;

public class PDFDirectorySource(string sourceDirectory) : IIngestionSource
{
    public static string SourceFileId(string path) => System.IO.Path.GetFileName(path);
    ...
    {
        using var pdf = PdfDocument.Open(System.IO.Path.Combine(sourceDirectory, documentId));
        ...
        {
            Key = $"{System.IO.Path.GetFileNameWithoutExtension(documentId)}_{pair.First.PageNumber}_{pair.First.IndexOnPage}",
            FileName = documentId,
